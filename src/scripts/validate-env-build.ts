...
function main() {
  ...
  if (missing.length === 0) {
    console.log('[check:env:r2] Variáveis R2 obrigatórias presentes.');
    return;
  }
  const msg = formatR2AssetsMissingMessage(missing);
  console.error(`[check:env:r2] ERRO: ${msg}`);
  if (process.env.ALLOW_MISSING_R2 === '1') {
    console.warn(
      '[check:env:r2] ALLOW_MISSING_R2=1 — verificação ignorada (não usar em produção).',
    );
    return;
  }
  process.exit(1);
