...
export async function seedEntriesDB(app: App, seed: SeedResult): Promise<SeedEntries> {
  ...
  if (medEntry.status !== 200) throw new Error("Erro ao criar entrada de medicamento no seedEntriesDB");
  ...
  if (inputEntry.status !== 200) throw new Error("Erro ao criar entrada de insumo no seedEntriesDB");
