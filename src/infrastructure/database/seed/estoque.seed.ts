...
export async function seedDB(app: App): Promise<SeedResult> {
  ...
  if (catRes.status !== 201) throw new Error("Erro ao criar categoria no seedDB");
  ...
  if (medRes.status !== 201) throw new Error("Erro ao criar medicamento no seedDB");
  ...
  if (inputRes.status !== 201) throw new Error("Erro ao criar insumo no seedDB");
  ...
  if (resRes.status !== 201) throw new Error("Erro ao criar residente no seedDB");
